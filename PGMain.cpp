...
void  PG::OutputLexicalSymbols ()
{
	...
	/* <lexical> symbols */
		...
		for (; i < N_terms; i++)
		{
			if (i == eof_term && optn[PG_EOFINCLUDED] == 0) continue;
			if (term_name[i][0] == '<')
			{
 				n++;
				write (filedesc, "\t", 1);
				k = strlen(term_name[i]);
				write (filedesc, term_name[i], k);
				ns = (max_terml - k + 1);
				if (ns > 0) write (filedesc, spaces, ns);
				k = sprintf (string, "=> %5d", i);
				write (filedesc, string, k);
			/*	if (term_const[i] >= 0)
				{
					k = sprintf (string, " %s", defcon_name[term_const[i]]);
					write (filedesc, string, k);
				}  */
				write (filedesc, "\n", 1);
			}
	...
	/* Keywords */
		...
		for (i = 1; i < N_terms; i++)
		{
			if (itsakeyword(term_name[i]))
			{
				...
				write (filedesc, "\t", 1);
				p   = slash_inside_keyword (term_name[i], quote);
				len = strlen (p);
				write (filedesc, p, len);
				ns = (max_terml - len + 1);
				if (ns > 0) write (filedesc, spaces, ns);
				len = sprintf (string, "=> %5d", i);
				write (filedesc, string, len);
				write (filedesc, "\n", 1);
			}
		}
	...
	/* 'literal' symbols */
		...
		for (i = 1; i < N_terms; i++)
		{
			if (term_name[i][0] == '{') continue;
			if (term_name[i][0] == '<') continue;
			if (itsakeyword (term_name[i])) continue;
			if (first)
			{
				first = 0;
				write (filedesc, "\n", 1);
