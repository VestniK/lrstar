...
int   main (int na, char *arg[])
{
   ...
   // Initializations ...
		...
		if (!calc_parser::init (Input.filename, Output.filedesc, 10000, 100000)) { rc = 4; goto Ret; }
		
   // Parsing ...
		printf  (                 "calc_parser\n");
		fprintf (Output.filedesc, "calc_parser\n");
		printf  (                 "Parsing %s ...\n\n", Input.filename);
		fprintf (Output.filedesc, "Parsing %s ...\n\n", Input.filename);

		for (int i = 0; i < 1; i++)
		{
			int n = calc_parser::parse (Input.input_start);
			if (n <= 0) 
			{
				nl = -n;
				goto Done;
