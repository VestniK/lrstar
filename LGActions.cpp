...
int   LGActions::PRO_OPTNS (int prod, int a) 
{
		...
		if (x > 0)
		{
			int la = get_lookahead();
			if (la == T_RETURN) // If lookahead is a '=>', this <string> is NOT an option. 
			{
			// Back up one token ...
				col_numb -= token.end - token.start;
...
Top:			while (*p == ' ' || *p == '\t' || *p == ',') p++;   // bypass spaces, tabs & commas.
				opt = p;
				while (*p != '\n' && *p != 26 && *p != '"' && *p != ',') p++;
			// Not likely to to crash here, parser would have given an invalid symbol error first. 
				c = *p;
				*p = 0;
...
int   LGActions::term ()
{
   ...
   // Count the number of terminals ...
      ...
      {  
         if (!(sym_type[s] & (NONTERMINAL|CONSTANT|STRING))) 
         {  
...
//			printf ("%s [%d] type = %d\n", head_name[h], s, head_type[h]);
   ...
   /* Assign Tail numbers. */
	...
	// Define Global variables (remove this later after making everything OOD. 
		n_symbs     = n_symbs;
...
int   LGActions::GEN_STRINGS ()   /* Generate strings array. */
{
      int s, i = 0;
